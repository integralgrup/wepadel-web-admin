                ...
                <div class="card">
                    ...
                    <?php 
                        foreach($slider as $item){
                            $blog_id[$item->lang] = $item->blog_id;
                            $image[$item->lang] = $item->media_file;
                            $alt[$item->lang] = $item->alt;
                        }
                    ?>
                    <div class="card-body">
                        <form action="{{ route('admin.blog.slider.store', $blog_id[$item->lang]) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                                    ...
                                    <div class="card-body" style="display:grid; grid-template-columns: 1fr 1fr 1fr; gap: 20px;">
                                        <div class="form-group">
                                            ...
                                            @if(isset($image[$language->lang_code]))
                                                <img src="{{ $language->domain .'/'. getFolder(['uploads_folder', 'blog_images_folder'], $language->lang_code) .'/'.$image[$language->lang_code] }}" alt="{{ $alt[$language->lang_code] }}" style="width: 100px; margin-top: 10px;">
                            ...
                            <div class="form-group mt-3">
                                ...
                                <a href="{{ route('admin.blog.slider.index', $blog_id[$item->lang]) }}" class="btn btn-secondary">Geri Dön</a>
                            </div>
                        </form>
                    </div>
                </div>
